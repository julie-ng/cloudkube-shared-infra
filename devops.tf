...
resource "azurerm_key_vault" "devops-staging" {
  name                       = "${var.base_name}-devops-staging-kv"
  location                   = var.location
  sku_name                   = var.key_vault_sku
  resource_group_name        = var.shared_rg_name
  tenant_id                  = data.azurerm_client_config.current.tenant_id
...
// staging
resource "azuread_application" "ci_staging_kv_reader" {
  display_name = "${var.base_name}-ci-staging-kv-reader-sp"
  owners       = [data.azuread_client_config.current.object_id]
}
...
// staging
resource "azuread_service_principal" "ci_staging_kv_reader" {
  application_id               = azuread_application.ci_kv_reader.application_id
  app_role_assignment_required = false
  owners                       = [data.azuread_client_config.current.object_id]
}
...
// staging
resource "azuread_service_principal_password" "ci_staging_kv_reader" {
  service_principal_id = azuread_service_principal.ci_staging_kv_reader.object_id
}
...
resource "azurerm_key_vault_secret" "ci_staging_kv_reader_client_id" {
  name         = "${var.base_name}-ci-staging-kv-reader-client-id"
  value        = azuread_service_principal.ci_kv_reader.application_id
  key_vault_id = azurerm_key_vault.devops.id
}

resource "azurerm_key_vault_secret" "ci_staging_kv_reader_client_secret" {
  name         = "${var.base_name}-ci-staging-kv-reader-client-secret"
  value        = azuread_service_principal_password.ci_kv_reader.value
  key_vault_id = azurerm_key_vault.devops.id
}
